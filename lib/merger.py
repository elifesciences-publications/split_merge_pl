...
class Merger:
    ...
    def merge_all_files(self, lsample, suffix, Merge_dir):
        ...
        ## Remove the split files in the Split_dir
        remove_split = cldict.remove_split
        if remove_split:
            # Adapted from https://stackoverflow.com/a/185941
            # code of adaptation starts
            for the_file in os.listdir(Split_dir):
                file_path = os.path.join(Split_dir, the_file)
                try:
